# Adapted from https://github.com/TIBHannover/GeoEstimation/blob/master/download_images.py
...
def flickr_download(x, size_suffix="z", min_edge_size=None):
    ...
    if "http" in x["url"]:
        ...
        try:
            image = PIL.Image.open(x['url'])
        except:
            print(f"Error opening {x['url']}")
            return
...
class ImageDataloader:
    def __init__(self, url_csv: Path, shuffle=False, nrows=None, prefix=""):

        logger.info("Read dataset")
        self.df = pd.read_csv(
            url_csv, nrows=nrows
        )
        if "IMG_ID" not in self.df:
            self.df.columns = ["image_id", "url"]
        else:
            self.df = self.df[["IMG_ID"]]
            self.df["image_id"] = self.df["IMG_ID"]
            self.df["url"] = prefix + self.df["IMG_ID"]
            self.df = self.df.drop(["IMG_ID"], axis=1)
...
def main():
    ...
    if not os.path.exists(args.image_prefix):
        os.makedirs(args.image_prefix)

    image_loader = ImageDataloader(args.url_csv, nrows=args.nrows, shuffle=args.shuffle, prefix=args.image_prefix)
